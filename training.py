...
def train():
    ...
    if option == 'teach':
        ...
        super_keywords = str(
            input("[Super Keywords] This is searched first (sep=,): ")).strip()
        anti_keywords = str(input(
            "[ANTI keywords]These keywords will abort the execution process (sep=,): ")).strip()
        script_function = str(input(
            "Make sure to check with your scripts in ai-actions.py (e.g. move_folder): ")).strip()
    # Creates Table
    # keywords is empty this is handled by the engine
        ...
        c.execute(
            f'''INSERT INTO AI(name, super_keywords, keywords, antikeywords, script_function) VALUES(?, ?, ?, ?, ?)''', ai_entry)
        ...
        name = rows[option][0]
        columns = ['name', 'super_keywords', 'keywords',
                   'antikeywords', 'script_function']
        index = 0
        ...
        c.execute(
            f'UPDATE AI SET {edit_what} = "{edit}" WHERE name = "{name}"')
        done()
    elif option == 'unteach':
